...
def calc():
    ...
    adware = datasets.load_all(os.path.join("datasets", DATASET_NAME))  # load dataset from csv
    logger.info("{} {}".format("benign shape", benign.shape))
    logger.info("{} {}".format("adware shape", adware.shape))

    benign = datasets.prepare_dataset(benign, shuffle=True)
    adware = datasets.prepare_dataset(adware, shuffle=True)

    n_elements = min(benign.shape[0], adware.shape[0], 150000)

    benign = benign.head(n_elements)
    adware = adware.head(n_elements)

    logger.info("{} {}".format("benign shape after balancing", benign.shape))
    logger.info("{} {}".format("adware shape after balancing", adware.shape))

    adware["Label"] = DATASET_NAME.upper()

    loaded_dataset = pd.concat([benign, adware], ignore_index=True)  # union dataset
